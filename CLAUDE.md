...
- ドメイン分離:
  - 公式コンテンツ: `Course` / `Lesson` (table 名 `courses` / `lessons`)
  - UGC: `Collection` / `Problem` (table 名 `collections` / `problems`)
- 現スタック: Next.js 16 (App Router, Turbopack) / React 19 / Tailwind v4 / shadcn/ui / Prisma 7 (driver adapter: `@prisma/adapter-pg`) / Supabase Postgres + Auth / Monaco Editor / esbuild-wasm / next-safe-action / SWR / biome / pino / lucide-react / zod / react-hook-form。
- ローカル shadow DB のみ Docker Postgres (`prisma migrate dev` 用)。runtime DB は Supabase Postgres。
...
| ファイル | 内容 |
| :-- | :-- |
| [`.claude/rules/architecture.md`](./.claude/rules/architecture.md) | 5 レイヤードアーキテクチャ（Presentation / API / Service / Repository / Data）、`src/` 11 ディレクトリ固定、認証方針 (`(protected)` + `requireAuth`/`requireRole` + `actionClient` middleware)、認可の 4 層 (Page / Action / Service / Route Handler)、URL 名前空間 3 区分 (app 全体共有 / ユーザー所有 / session 私有)、DB / Prisma 規約、Import 順、コード例 |
| [`.claude/rules/tech-stack.md`](./.claude/rules/tech-stack.md) | 採用ライブラリと各々の使い分け。**導入済み**: Next.js 16 / React 19 / Tailwind v4 / shadcn/ui / Prisma 7 / Supabase / next-safe-action / SWR / biome / pino / lucide-react / server-only / react-hook-form (+ `@hookform/resolvers`) / zod / esbuild-wasm。**未導入**: zustand / playwright / vitest / nuqs |
