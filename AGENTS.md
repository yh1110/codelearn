...
以下のファイルはすべて `alwaysApply: true` で、Claude Code セッションに常時 include される（`CLAUDE.md` 経由）。Cursor 等 `.claude/rules/` を直接読まないツールでも、下記パスを AI に明示的に読ませれば機能する。
...
1. **本ファイル（`AGENTS.md`）** — プロジェクト概要と Next.js 16 警告を頭に入れる。
...
4. **該当コード** — 編集対象のディレクトリで `ls` / `grep` を走らせ、既存パターンを確認する。一般的な TS/Node ベストプラクティス（`utils/`, `helpers/`, `mappers/` 等）を **ルールより優先しない**。
