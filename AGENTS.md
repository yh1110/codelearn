...
- **役割**: DB アクセス。
- **必須**: DB アクセスは **必ず** `src/features/<domain>/repository.ts` 経由。**`repository.ts` 以外のファイル**（Server Component / Server Action / API Route / Client Component 等）から Prisma Client を import しない。repository 層のみが Prisma を触る唯一の場所である。
...
- **移行フェーズ**: biome はまだ未導入。現状の lint は `eslint` (`npm run lint`) を維持する。
- **biome 導入後**: `npm run check`（lint）/ `npm run format`（format）を `package.json` に追加し、CI でも同じコマンドを通す。同タイミングで `eslint.config.mjs` と `eslint` 依存を削除する。
- **使わない**: ESLint / Prettier を biome と **併用** しない。移行は一括で行う。
